...
class SpeechBubbleClipper extends CustomClipper<Path> {
  ...
  get edgeInsets {
    if (nipDirection == NipDirection.LEFT_TOP
        || nipDirection == NipDirection.LEFT_CENTER
        || nipDirection == NipDirection.LEFT_BOTTOM) {
  ...
  Path getClip(Size size) {
    ...
    switch (nipDirection) {
      ...
      case NipDirection.RIGHT_BOTTOM:
        path.addRRect(RRect.fromLTRBR(
            _endOffset, 0, size.width - _startOffset, size.height, radius));
  ...
  SpeechBubble({
    ...
    required this.child
        ...
        clipper = SpeechBubbleClipper(
          ...
          radius: style.radius,
          nipDirection: nipDirection ?? NipDirection.LEFT_CENTER,
